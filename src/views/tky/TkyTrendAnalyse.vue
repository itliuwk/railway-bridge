  ...
  <a-card :bordered="false">
    ...
    <div class="table-page-search-wrapper">
      ...
      <a-input
        placeholder="请输入桥梁编号"
        style="width: 10%"
        v-model="queryParam.bridgeCode"
      ></a-input>

      病害名称：
      <a-select default-value="请选择" style="width: 10%" @change="handleChange">
        <a-select-option value="请选择"> 请选择 </a-select-option>
        <a-select-option value="裂缝"> 裂缝 </a-select-option>
        <a-select-option value="网状裂缝"> 网状裂缝 </a-select-option>
        <a-select-option value="夹块"> 夹块 </a-select-option>
        <a-select-option value="施工冷缝"> 施工冷缝 </a-select-option>
        <a-select-option value="剥落（掉块）"> 剥落（掉块） </a-select-option>
        <a-select-option value="渗漏水"> 渗漏水 </a-select-option>
        <a-select-option value="施工缝破损"> 施工缝破损 </a-select-option>
      </a-select>
      ...
      <a-input-number
        placeholder="起始里程"
        style="width: 8%"
        v-model="queryParam.startMil"
      />
      终止里程：
      <a-input-number
        placeholder="终止里程"
        style="width: 8%"
        v-model="queryParam.endMil"
      />
      起始版本：
      <a-input-number
        placeholder="大版图专用"
        style="width: 8%"
        v-model="queryParam.fullStartVersion"
      />
      终止版本：
      <a-input-number
        placeholder="大版图专用"
        style="width: 8%"
        v-model="queryParam.fullEndVersion"
      />
      <a-button type="primary" @click="trendAnalyse" style="margin-left: 3%"
        >分析</a-button
      >
    </div>
    <a-tabs @change="callback" :loading="loading">
      ...
      <a-tab-pane tab="长度趋势图" key="1">
        <line-chart-multid-len
          :dataSource="dataSourceLen"
          :fields="fieldsLen"
          :aliases="aliasesLen"
          :height="height"
        />
      ...
      <a-tab-pane tab="宽度趋势图" key="2">
        <line-chart-multid-wid
          :dataSource="dataSourceWid"
          :fields="fieldsWid"
          :aliases="aliasesWid"
          :height="height"
        />
      ...
      <a-tab-pane tab="大版趋势" key="4">
        <template>
          <a-list
            ...
            :data-source="dataSourceFull"
          >
            <a-list-item :key="index" slot="renderItem" slot-scope="record, index">
              <a href="javascript:void(0)" @click="getFullImage(record)">{{
                record.name
              }}</a>
            </a-list-item>
          </a-list>
...
export default {
  ...
  methods: {
    ...
    getFullImage(record) {
      ...
      getAction("/moudle/tkyImgdataFull/trendAnalyseFullTime", this.queryParam).then(
        (res) => {
    ...
    trendAnalyse() {
      if (
        this.key != "4" &&
        (this.queryParam.type == "请选择" ||
          this.queryParam.bridgeCode == "" ||
          this.queryParam.startMil == "" ||
          this.queryParam.endMil == "")
      ) {
      ...
      if (this.queryParam.type == "裂缝" || this.queryParam.type == "施工冷缝") {
        ...
        if (this.key == "3")
          getAction("/moudle/tkyDisease/trendAnalyseArea", this.queryParam).then(
            (res) => {
      ...
      if (this.key == "4")
        getAction("/moudle/tkyImgdataFull/trendAnalyseFull", this.queryParam).then(
          (res) => {
